...
UnicodeにはCOMBINING ENCLOSING KEYCAP(U+20E3 ` ⃣`)という[結合文字]があり、他の文字と組み合わせてキーボードなどボタン上に書かれた文字を表現するのに用いられる。この中で電話のダイヤルボタンに用いられるキー(`#`,` * `,`0`..`9`)にだけ絵文字表現が存在する。
